								...
								<a href="../ServerInfo/?name='.$top_list[$i].'">
									...
									<div class="row">
										<div class="col-2">
											<center>
												<font color="#000">
													<i class="fa fa-download"></i> 미지원<br>
													<i class="fa fa-retweet"></i> '.($peak1_json->players > 0 ? (round($peak2_json->players / $peak1_json->players * 10000) / 100) - 100 : 0).'%<br>
													<i class="fa fa-globe"></i> '.(round($info_json->players / getJSON("../API/Players.json")->players * 10000) / 100).'%
												</font>
											</center>
										</div>
										<div class="col-10">
											<br>
											<h5>'.replaceColor($info_json->motd).'</h5>
										</div>
									</div>
									<br>
									<div class="row">
										...
										<div class="col-6">
											<p class="text-right">
												<span class="badge badge-success"><i class="fa fa-users"></i> '.$peak_json->players.'</span>
												<span class="badge badge-info">'.$info_json->players.' / '.$info_json->maxplayers.'</span>
											</p>
										</div>
									</div>
								</a>
							...
							<li class="list-group-item">
								<center>
									<div class="btn-group" role="group">
										<button type="button" class="btn btn-'.($page == 1 ? 'secondary' : 'success').'" onClick="location.href=\'../Test/?page=1\'">맨 앞</button>';
		if ($page - 1 > 0) $message = $message.'<button type="button" class="btn btn-info" onClick="location.href=\'../Test/?page='.($page - 1).'\'"><</button>';
		for (; $i <= $m; $i++) {
			$message = $message.'
										<button type="button" class="btn btn-'.($page == $i ? 'outline-' : '').'primary" onClick="location.href=\'../Test/?page='.$i.'\'">'.$i.'</button>
								';
		}
		if ($page + 1 < $top_page) $message = $message.'<button type="button" class="btn btn-info" onClick="location.href=\'../Test/?page='.($page + 1).'\'">></button>';
		$message = $message.'
										<button type="button" class="btn btn-'.($page == $top_page ? 'secondary' : 'success').'" onClick="location.href=\'../Test/?page='.$top_page.'\'">맨 뒤</button>
									</div>
								</center>
							</li>
							';
