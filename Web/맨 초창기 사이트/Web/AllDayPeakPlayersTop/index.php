<?php
	...
	$page_name = "최고 동시 접속자 순위";
	
	$page = 1;
	
	if (!empty($_GET['page'])) $page = $_GET['page'];
	
	$top_list = getJSON("../API/Peak/AllDayTop.json")->list;
							...
							<li class="list-group-item">
								<center>
									<div class="btn-group" role="group">
										<button type="button" class="btn btn-'.($page == 1 ? 'secondary' : 'success').'" onClick="location.href=\'../AllDayPeakPlayersTop/?page=1\'">맨 앞</button>';
		if ($page - 1 > 0) $message = $message.'<button type="button" class="btn btn-info" onClick="location.href=\'../AllDayPeakPlayersTop/?page='.($page - 1).'\'"><</button>';
		for (; $i <= $m; $i++) {
			$message = $message.'
										<button type="button" class="btn btn-'.($page == $i ? 'outline-' : '').'primary" onClick="location.href=\'../AllDayPeakPlayersTop/?page='.$i.'\'">'.$i.'</button>
								';
		}
		if ($page + 1 < $top_page) $message = $message.'<button type="button" class="btn btn-info" onClick="location.href=\'../AllDayPeakPlayersTop/?page='.($page + 1).'\'">></button>';
		$message = $message.'
										<button type="button" class="btn btn-'.($page == $top_page ? 'secondary' : 'success').'" onClick="location.href=\'../AllDayPeakPlayersTop/?page='.$top_page.'\'">맨 뒤</button>
									</div>
								</center>
							</li>
							';
