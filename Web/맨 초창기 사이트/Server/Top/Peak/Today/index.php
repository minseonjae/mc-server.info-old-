<?php
	...
	$top_list = getJSON("../../../../API/TodayPlayersTop.json")->top;
							...
							<a href="../../../../Server/?name='.$top_list[$i].'" class="list-group-item">
								...
								<div class="row">
									...
									<div class="col-xs-5 col-md-6">
										<p class="text-right">
											<span class="badge badge-success" data-toggle="tooltip" data-placement="top" title="오늘 최고 동시 접속자<br>'.$data_json->peak->today->time.'" data-html="true">'.$data_json->peak->today->players.'명</span>
							...
							<li class="list-group-item">
								<center>
									<div class="btn-group" role="group">
										<button type="button" class="btn btn-default '.($page == 1 ? 'disabled' : '').'" onClick="location.href=\'../Today/?page=1\'">맨 앞</button>';
		if ($page - 1 > 0) $message = $message.'<button type="button" class="btn btn-info" onClick="location.href=\'../Today/?page='.($page - 1).'\'"><</button>';
		for (; $i <= $m; $i++) {
			$message = $message.'
										<button type="button" class="btn btn-default '.($page == $i ? 'disabled' : '').'" onClick="location.href=\'../Today/?page='.$i.'\'">'.$i.'</button>
								';
		}
		if ($page + 1 < $top_page) $message = $message.'<button type="button" class="btn btn-info" onClick="location.href=\'../Today/?page='.($page + 1).'\'">></button>';
		$message = $message.'
										<button type="button" class="btn btn-default '.($page == $top_page ? 'disabled' : '').'" onClick="location.href=\'../Today/?page='.$top_page.'\'">맨 뒤</button>
									</div>
								</center>
							</li>
							';
					...
					<div class="row">
						<div class="col-md-12">
							...
							<div class="list-group">
								<div class="list-group-item">오늘 최고 동시 접속자 순위</div>
								<?php echo $message; ?>
							</div>
						</div>
					</div>
